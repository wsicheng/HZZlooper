...
def plotBackgrounds(h_bkg_vec_, bkg_names, canvas=None, stack=None, saveAs=None, xRangeUser=None, doPause=False, 
    ...
    if xAxisUnit==None:
        ...
        stack.GetXaxis().SetTitle(xAxisTitle + " ({0})".format(xAxisUnit)) # FIXME: temporary for 2l2nu plots only
...
def plotRatio(h1, h2, canvas=None, ratioHist=None, xRangeUser=None, ratioTitle = None, markerSize=0.7, markerStyle=20, 
              ...
              justDrawPoints=False, drawMCerr=False):
    ...
    if not doPull:
        ...
        ratioHist.Divide(h1)
        if convertToPoisson:
            utils.GetPoissonRatioGraph(h1,h2,ratioGraph,drawZeros=drawZeros, useMCErr=(not drawMCerr))
        elif isEffic:
    ...
    if not doPull:
        ...
        elif yRangeUser == True:
            maxdiff = 0.2;
            for ibin in range(1, ratioHist.GetNbinsX()+1):
                maxdiff = max(maxdiff, abs(ratioHist.GetBinContent(ibin)-1+ratioHist.GetBinError(ibin)))
                if drawSystematicBand: maxdiff = max(maxdiff, systematics[ibin-1])
            maxdiff = round(maxdiff*12+0.5) / 10
            ratioHist.GetYaxis().SetRangeUser(max(0, 1-maxdiff), min(2, 1+maxdiff))
        else:
    ...
    ratioHist.GetYaxis().SetTitleSize(0.17) # temporary: 2l2nu only
    ratioHist.GetYaxis().SetTitleOffset(0.26) # temporary: 2l2nu only
    ...
    if drawSystematicBand:
        ...
        h1.Copy(h_syst)
        for i in range(1, h_syst.GetNbinsX()+1):
            h_syst.SetBinError(i, 1.0*h_syst.GetBinError(i)/h_syst.GetBinContent(i) if h_syst.GetBinContent(i) > 0 else 0)
            h_syst.SetBinContent(i,1)
        h_syst.SetFillStyle(3244)
        h_syst.SetFillColor(ROOT.kGray+3)
        h_syst.Draw("SAME E2")
...
def plotDataMC(h_bkg_vec_, bkg_names, h_data=None, title=None, subtitles=None, ratioTitle=None, doRatio=True, yRangeUserRatio=None, 
               ...
               titleSize=0.04, subtitleSize=0.03, subLegText=None, subLegTextSize=0.03, cmsText="Preliminary", cmsTextInside=False,
               cmsTextSize=0.048, doBkgError=False, functions=[], legCoords=None, legNCol=1, drawLegBox=True, doPull=False, 
               convertToPoisson=False, drawZeros=True, drawSystematicBand=False, systematics=None, h_sig_vec=[], sig_names=[], 
               scaleSigToMC=False, customColors=None, verticalLines=[], ratioType=0, ratioEffic=False, doComposition=False):    
    ...
    if doRatio:
        ...
        pads = []
        pads.append(ROOT.TPad("1","1",0.0,0.17,1.0,1.0))
    ...
    if(h_data!=None):
        ...
        scaleFactor = data_integral/tot_MC_integral if tot_MC_integral != 0 else 1.0
        if type(scaleMCtoData) == float:
            scaleFactor = scaleMCtoData
        if data_integral > 0:
    ...
    ## signals
    sig_cols = [  ROOT.kTeal, ROOT.kOrange+7, ROOT.kViolet, ROOT.kCyan, ROOT.kMagenta, ROOT.kGreen]
    ...
    if cmsText is not None:
        utils.DrawCmsText(pads[0],text=cmsText,textFont=52,textSize=cmsTextSize, insideAxes=cmsTextInside)
    ...
    if doRatio:
        ...
        h_syst = ROOT.TH1D()
        hasDrawn = False
        if ratioType==1 and yRangeUserRatio==None: yRangeUserRatio = True
        for ih,hd in list(enumerate(h_data))[::-1]:
            if ratioType==0:
                plotRatio(h1, hd, canvas=pads[1], ratioHist=ratioHist[ih], ratioTitle=ratioTitle, xRangeUser=xRangeUser, 
                          ...
                          justDrawPoints=hasDrawn, drawMCerr=True)
                hasDrawn = True
            else:
                if len(h_data) <= 1:
                ...
                hden = h_data[0]
                hnum = hd
                if ratioTitle=="Data/MC": ratioTitle="Obs./Sim."
    ...
    if doComposition:
        pads[1].cd()
        plotComposition(stack, canvas=pads[1], justDrawPoints=doRatio)
...
## make the composition of a stack (i.e. normalize every bin to 1), should, can be added to the ratio plot, or replace the main plot
def plotComposition(stack, canvas=None, hdata=None, xRangeUser=None, ratioTitle = None, markerSize=0.7, markerStyle=20, 
                    markerColor=ROOT.kBlack, convertToPoisson=False, ratioGraph=None, drawZeros=True,
                    drawSystematicBand=False, systematics=None, h_syst=None, yRangeUser=None, justDrawPoints=False):

    nst = ROOT.THStack("hs2","")
    htot = None
    # hlst = []
    for h in stack.GetHists():
        nh = h.Clone()
        if htot==None:
            htot = nh.Clone('stacktot')
        else:
            htot.Add(nh)
        # hlst.append(nh)

    for h in stack.GetHists():
        nh = h.Clone(h.GetName()+"_compo")
        for ibin in range(1, h.GetNbinsX()+1):
            ibt = htot.GetBinContent(ibin)
            nh.SetBinContent(ibin, 0. if ibt <= 0. else 1.*h.GetBinContent(ibin) / ibt)
            if nh.GetBinContent(ibin) > 1: nh.SetBinContent(ibin, 1.0)
            # print ibin, ibt, h.GetBinContent(ibin),  nh.GetBinContent(ibin)
            nh.SetBinError(ibin, 0. if ibt <= 0. else 1.*h.GetBinError(ibin) / ibt)
...
def plotComparison(h1_, h2_, title="", ratioTitle="Data/MC", h1Title="MC", h2Title="Data", saveAs=None,
                   ...
                   cmsText="Preliminary", energy=13, extraEnergyText=None, subLegText=None,
